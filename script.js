...
var E48 = [1.00, 1.05, 1.10, 1.15, 1.21, 1.27, 1.33, 1.40, 1.47, 1.54, 1.62, 1.69, 1.78, 1.87, 1.96, 2.05, 2.15, 2.26, 2.37, 2.49, 2.61, 2.74, 2.87, 3.01, 3.16, 3.32, 3.48, 3.65, 3.83, 4.02, 4.22, 4.42, 4.64, 4.87, 5.11, 5.36, 5.62, 5.90, 6.19, 6.49, 6.81, 7.15, 7.50, 7.87, 8.25, 8.66, 9.09, 9.53];
var E96 = [1.00, 1.02, 1.05, 1.07, 1.10, 1.13, 1.15, 1.18, 1.21, 1.24, 1.27, 1.30, 1.33, 1.37, 1.40, 1.43, 1.47, 1.50, 1.54, 1.58, 1.62, 1.65, 1.69, 1.74, 1.78, 1.82, 1.87, 1.91, 1.96, 2.00, 2.05, 2.10, 2.15, 2.21, 2.26, 2.32, 2.37, 2.43, 2.49, 2.55, 2.61, 2.67, 2.74, 2.80, 2.87, 2.94, 3.01, 3.09, 3.16, 3.24, 3.32, 3.40, 3.48, 3.57, 3.65, 3.74, 3.83, 3.92, 4.02, 4.12, 4.22, 4.32, 4.42, 4.53, 4.64, 4.75, 4.87, 4.99, 5.11, 5.23, 5.36, 5.49, 5.62, 5.76, 5.90, 6.04, 6.19, 6.34, 6.49, 6.65, 6.81, 6.98, 7.15, 7.32, 7.50, 7.68, 7.87, 8.06, 8.25, 8.45, 8.66, 8.87, 9.09, 9.31, 9.53, 9.76];
...
function computeRes(ratio) {

    var result, final_result, R1i, R2i, decade1, decade2, error, min_error, best_R1, best_R2, R1, R2, qcurrent, target_qcurrent, best_qcurrent, qcurrent_error, min_qcurrent_error;
    var voltage = 10;

    target_qcurrent = 0.001;

    for (R1i = 0; R1i < eseries.E192.length; R1i++) {
        for (R2i = 0; R2i < eseries.E192.length; R2i++) {
            for (decade1 = 1; decade1 < 8; decade1++) {
                for (decade2 = 1; decade2 < 8; decade2++) {
                    R1 = eseries.E192[R1i] * (10 ** decade1);
                    R2 = eseries.E192[R2i] * (10 ** decade2);
                    result = (R2 / R1);
                    error = Math.abs(ratio - result) / ratio;
    ...
    alert("result: " + final_result + " R1: " + best_R1 + " R2: " + best_R2 + " error: " + min_error * 100 + "% Quiescent Current (A): " + best_qcurrent);

}

function computeVoltageDivider() {
        ...
        for (R2i = 0; R2i < eseries.E192.length; R2i++) {
            for (decade1 = 1; decade1 < 8; decade1++) {
                for (decade2 = 1; decade2 < 8; decade2++) {
                    ...
                    error = Math.abs(vout - result) / vout;
                    qcurrent = voltage / (R1 + R2);
                    qcurrent_error = Math.abs(qcurrent - target_qcurrent) / target_qcurrent;
                    if (((error <= min_error) && (qcurrent_error <= min_qcurrent_error)) || ((min_error == undefined) || (min_qcurrent_error == undefined))) {
                        min_error = error;
    ...
    var output = {v_out:final_result, R1:R1, R2:R2, qcurrent:best_qcurrent};
    return output;
    alert("result: " + final_result + " R1: " + best_R1 + " R2: " + best_R2 + " error: " + min_error * 100 + "% Quiescent Current (A): " + best_qcurrent);
